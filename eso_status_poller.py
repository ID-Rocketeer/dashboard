...
CHROME_USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36"

# A list of the specific, exact keys we want from the API response's 'response' object
TARGET_SERVERS = [
...
def fetch_eso_status():
    ...
    try:
        ...
        # Navigate to the 'response' dictionary which holds the status strings
        # This handles the specific structure you reported: data['zos_platform_response']['response']
        try:
            ...
            return {
                ...
                "MESSAGE": "API keys zos_platform_response or response not found."
            }
        ...
        for server_key in TARGET_SERVERS:
            ...
            if server_key in realms:
                # The status is the VALUE (a string, e.g., "UP") associated with the key
                status = realms[server_key].upper()
                
                # Format the key for cleaner dashboard display (e.g., "NA" or "EU")
                # Strips the "The Elder Scrolls Online ()" part
                display_key = server_key.replace("The Elder Scrolls Online", "").strip()
                display_key = display_key.replace("(", "").replace(")", "").strip()
        ...
        return {"STATUS": "HTTP ERROR", "MESSAGE": f"Failed to connect (HTTP {e.response.status_code})"}
